...
class FlatFileSystem : public LogicalFileSystem
{
    public:
        ...
        plfs_error_t getattr(struct plfs_physpathinfo *ppip,
                    struct stat *stbuf,int sz_only, Plfs_open_opt *oopt);
        plfs_error_t trunc(struct plfs_physpathinfo *ppip, off_t offset, int open_file, Plfs_open_opt *oopt);
        ...
        plfs_error_t create(struct plfs_physpathinfo *ppip,
                   mode_t, int flags, pid_t pid, Plfs_open_opt *oopt);
